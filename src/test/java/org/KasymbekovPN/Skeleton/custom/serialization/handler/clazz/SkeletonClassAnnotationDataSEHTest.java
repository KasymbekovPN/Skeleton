...
public class SkeletonClassAnnotationDataSEHTest {
    ...
    void test(Class<?> clazz,
        ...
        SkeletonClassAnnotationDataSEH seh = new SkeletonClassAnnotationDataSEH(new SkeletonAnnotationChecker());
    ...
    private static class TestCollectorProcess implements CollectorProcess{
        ...
        private boolean checkModifiers(SkeletonObjectNode node, String property, int modifiers){
            if (node.containsKey(property)){
                Node modifiersNode = node.getChildren().get(property);
                if (modifiersNode.isNumber()){
                    return ((SkeletonNumberNode)modifiersNode).getValue().equals(modifiers);
                }
            }
        ...
        private boolean checkNames(SkeletonObjectNode node, String property, String[] names){
            if (node.containsKey(property)){
                Node namesNode = node.getChildren().get(property);
                if (namesNode.isArray()){
                    List<String> nodeNames = new ArrayList<>();
                    for (Node child : ((SkeletonArrayNode) namesNode).getChildren()) {
                        if (child.isString()){
                            nodeNames.add(((SkeletonStringNode)child).getValue());
                        } else {
                            return false;
                        }
                    }
