...
public class SkeletonConstructorClassSEHTest {
    ...
    private static class TestCollectorProcess implements CollectorProcess{
        ...
        public void handle(Node node) {
            ...
            if (maybeChild.isPresent() && maybeChild.get().isObject()){
                SkeletonObjectNode constructorsNode = (SkeletonObjectNode) maybeChild.get();

                valid = true;
                for (Map.Entry<String, Node> entry : constructorsNode.getChildren().entrySet()) {
                    if (entry.getValue().isArray()){
                        SkeletonArrayNode paramsNode = (SkeletonArrayNode) entry.getValue();
                        List<String> params = new ArrayList<>();
                        for (Node child : paramsNode.getChildren()) {
                            if (child.isString()){
                                params.add(((SkeletonStringNode)child).getValue());
                            } else {
                                valid = false;
                                break;
                            }
