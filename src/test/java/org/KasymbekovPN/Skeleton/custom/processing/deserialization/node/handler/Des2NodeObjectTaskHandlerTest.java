...
@DisplayName("Des2NodeObjectTaskHandler. Testing of")
public class Des2NodeObjectTaskHandlerTest {
    ...
    void testCheckMethod() throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ContextStateCareTakerIsEmpty {
        Tested tested = new Tested(UHandlerIds.OBJECT);
    ...
    void testDoItMethod(String line, boolean isNull, EntityItem ei) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ContextStateCareTakerIsEmpty {
        Tested tested = new Tested(UHandlerIds.OBJECT);
        ...
        tested.doIt(context);
        boolean checkedIsNull = context.getContextStateCareTaker().peek().getNode() == null;
        Assertions.assertThat(checkedIsNull).isEqualTo(isNull);
        if (!checkedIsNull){
            Assertions.assertThat(context.getContextStateCareTaker().peek().getNode().is(ei)).isTrue();
        }
    }

    private static class Tested extends Des2NodeObjectTaskHandler{
